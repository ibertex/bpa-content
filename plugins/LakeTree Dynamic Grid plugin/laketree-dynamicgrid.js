  ...
  try {
    ...
    for (let index = 0; index < columnConfig.length; index++) {
      ...
      if (columnConfig[index]["align"] != undefined && columnConfig[index]["name"] != undefined) {
        //if align is set, add a span with the align class
        columnConfig[index]["name"] = gridjs.html("<span class='align" + columnConfig[index]["align"].toLowerCase() + "'>" + columnConfig[index]["name"] + "</span>");
      }

      if (columnConfig[index]["format"] != undefined){
        columnConfig[index]["formatter"] = (_, row) => gridjs.html(this.formatCell(row,columnConfig[index]["format"],(columnConfig[index]["type"] == undefined) ? "string" : columnConfig[index]["type"],(columnConfig[index]["align"] == undefined) ? "" : columnConfig[index]["align"]));
      }
    }
